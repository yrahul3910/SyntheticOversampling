...
if __name__ == '__main__':
    ...
    rxs = [rx for rx in rxs if rx[1] not in ['Howso1', 'Howso2', 'SDV_FASTML', 'DAZZLE1', 'DAZZLE2']]
    rxs = [(rx[0], 'datasynthesizer' if rx[1] == 'DS' else rx[1].lower(), f'{rx[2]}*' if rx[0] == min([rx[0] for rx in rxs]) else rx[2]) for rx in rxs]
